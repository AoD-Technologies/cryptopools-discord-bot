...
const triggerChecks = hooks.reduce((curr = [], i) => {
...
function chatHooks (message) {
  triggerChecks.forEach((trigger) => {
    if (message.content.indexOf(trigger) > -1) {
      const hook = hooks.find(h => h.triggers.includes(trigger))
      if (hook) {
        const random = Math.floor(Math.random() * ((hook.responses.length - 1) + 1))
