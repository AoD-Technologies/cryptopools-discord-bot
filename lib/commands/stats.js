...
async function runCommand (message, args, client) {
  ...
  embed.setThumbnail('https://media.giphy.com/media/4H3Ii5eLChYul9p7NL/giphy.gif')
    ...
    response.on('end', async () => {
      try {
        ...
        if (args.length === 0) {
          ...
          embed.addField('Daemon Version', relatedPoolData[0][1].poolStats.networkVersion)
          embed.addField('Block Height', relatedPoolData[0][1].poolStats.networkBlocks)
          if (relatedPoolData.length === 1) {
            embed.setDescription(`Detailed ${coin.substr(0, 1).toUpperCase()}${coin.substr(1)} Pool Statistics`)

            embed.addField('Network Difficulty', getReadableHashRateString(relatedPoolData[0][1].poolStats.networkDiff))
            embed.addField('Total Hash Rate', getReadableHashRateString(relatedPoolData[0][1].hashrate + relatedPoolData[0][1].solohashrate))
            embed.addField('Total Miners', relatedPoolData[0][1].workerCount)
            embed.addField('Blocks Pending', relatedPoolData[0][1].blocks.pending)
            embed.addField('Blocks Confirmed', relatedPoolData[0][1].blocks.confirmed)
            embed.addField('Blocks Orphaned', relatedPoolData[0][1].blocks.orphaned)
          } else {
            const topFivePools = relatedPoolData.sort(([, { workerCount: aWorkers, hashrate: aHashRate, solohashrate: aSoloHashRate }], [, { workerCount: bWorkers, hashrate: bHashRate, solohashrate: bSoloHashRate }]) => sortDescendingByPopularity(aWorkers, aHashRate, aSoloHashRate, bWorkers, bHashRate, bSoloHashRate)).slice(0, 5)

            embed.setDescription(`Detailed ${coin.substr(0, 1).toUpperCase()}${coin.substr(1)} Multi-Algorithm Pool Statistics`)

            embed.addField('\u200b', '\u200b')
            topFivePools.forEach(([, details], i) => {
